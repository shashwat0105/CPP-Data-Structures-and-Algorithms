...
public:
    bool helper(vector<vector<char>>& board, string word, int i, int j, int n, int m, int k){
        ...
        board[i][j] = '.'; // when matches I make it a dot
        bool temp = false;
        int x[4] = {0,0,-1,1};
        int y[4] = {-1,1,0,0};
        ...
        board[i][j] = word[k]; // backtrack to make board return to its old state
        return temp;
    }

    bool exist(vector<vector<char>>& board, string word) {
        // similar to rat and maze problem
        ...
        for(int i=0;i<n;i++){
            for(int j=0;j<m;j++){
                if(word[0]==board[i][j]){  // call from where first letter matches
